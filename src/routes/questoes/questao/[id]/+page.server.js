...
export async function load({ params }) {
	...
	const questao = await db.select().from(table.questao).where(eq(table.questao.id, id)).limit(1);

	if (!questao.length) {
		throw error(404, 'Questão não encontrada.');
	}

	const categorias = await db.select({ nome: table.categoria.nome }).from(table.questao_categoria).innerJoin(table.categoria, eq(table.questao_categoria.id_categoria, table.categoria.id)).where(eq(table.questao_categoria.id_questao, id));
